...
### Time complexity non-sense
(best case; worst case; time complexity)

Selection Sort (Find the biggest; swap)
- If there are n items and i items have been sorted, how many pairs of items are gonna be compared? n - (i+1)
- for the helper function `get_index_of_smallest` there is no best or worst case
	- Always linear
- The `selection_sort` function, the biggest size that i gets to is n - 1 (obviously). So, i ends at n - 1
- i goes from 0 to n - 1. Each time `get_index_of_smallest` runs, it has `n - (i + 1)`. Each run of that has 1 comparison. So you would have $\sum^{n-1}_{i = 0} n - (i + 1)$ comparisons. That simplifies to n(n)
- So the whole aglo is quadratic.

Insertion sort (cards; weird; super weird)
- This one you have 2 assignments and 2 more assignments per i. So it's like 2 + i\*2 assignments for the helper function `(insert(L, i)`)
- The insert function takes longer when it has more swaps it needs to do (that is, it will swap each item until it finds a place. The further left, the more swaps. The smaller the number, the more swaps!)
- If the function is already sorted, it would be constant i think?
- If we're accounting assignments, the best case is constant (only 2 assignments; there ARE a bunch of comparisons still but no assignment is committed to)
- The insertion sort thingy itself still goes through all items to call insert() so it's linear


Now we do the same analysis for bubble sort!
