...
The solution is easy! Have 1 [[Clocked SR Latch]] (this is what sets and resets the value) and feed it directly into another clocked SR latch (with Q = S and $\bar Q = R$) such that when Q is set, the other latch will also be *set*.
...
- Flip-flops are *edge triggered* (they only work at an *edge* which is during the switch from 0 to 1 or 1 to 0 for C)
...
- This is a *negative-edge* triggered flip-flop (like the above SR one)
	- Means its output changes to D on the *falling-edge* -- when C goes from 1 to 0
