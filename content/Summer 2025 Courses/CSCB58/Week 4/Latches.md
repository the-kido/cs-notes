...
However, in the case where $\bar S$ and $\bar R$ are 0, the $Q_R$ and $Q_{T+1}$ haven't really settled on a value. The output becomes 1, 1, which is *forbidden* because it makes no sense.
- When $\bar S = \bar R = 0$, this is a *forbidden state*

> Essentially with S = R = 1, that is your forbidden state for any latch. Also, $S = R = 0$ implies no change happens; things hold their data
